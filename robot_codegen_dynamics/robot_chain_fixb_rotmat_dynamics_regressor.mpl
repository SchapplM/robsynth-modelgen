...
t_ges_minpar := t_ges_minpar:
u_ges_minpar := u_ges_minpar:
# Lagrange Formalismus (mit Funktion)
OutputLagrange := LagrangeN(t_ges_minpar, u_ges_minpar):
dTdqDdt_s := OutputLagrange[1]:
dTdq_s := OutputLagrange[2]:
dUdq_s := OutputLagrange[3]:
save dUdq_s, sprintf("../codeexport/%s_floatb_lagrange_dUdq_s_regressor_minpar_maple.m", robot_name):
save dTdq_s, sprintf("../codeexport/%s_floatb_lagrange_dTdq_s_regressor_minpar_maple.m", robot_name):
save dTdqDdt_s, sprintf("../codeexport/%s_floatb_lagrange_dTdqDdt_s_regressor_minpar_maple.m", robot_name):
# Extraktion einzelner Terme
# Gravitational Load
# Generate
taug_regressor_s := dUdq_s:
save taug_regressor_s, sprintf("../codeexport/%s_gravload_regressor_minpar_maple.m", robot_name):
# Matlab Export
...
save tau_regressor_s, sprintf("../codeexport/%s_invdyn_joint_fixb_regressor_minpar_maple.m", robot_name):
# Matlab Export
# Belastung der Gelenke
if codegen_act then
