...
# Ursache ist, dass der Parametervektor und damit der Regressor sich zwischen Fixed-Base und Floating-Base unterscheidet und daher ein Ableiten des Fixed-Base falles aus der Floating-Base-Formulierung mit einfacher Basis-Darstellung ("twist") nicht mˆglich ist.
# Autor
...
with(CodeGeneration):
with(StringTools):
# Einstellungen f¸r Code-Export: Optimierungsgrad (2=hˆchster) und Aktivierung jedes Terms.
codegen_opt := 2:
...
# Es ist in diesem Arbeitsblatt mˆglich, zwei verschiedene Regressoren zu generieren und zu exportieren: Basierend auf Minimalparametern und auf vollem Parametersatz (PV2).
...
for i to NQ do # Zeilenindex der Coriolismatrix
  for j to NQ do  # Spaltenindex der Coriolismatrix
    ...
    for k from 1 to Paramvec_size do # Spaltenindex der Regressormatrix
      # Massenmatrix f¸r Parameter k generieren (f¸r Funktion mit Christoffel-Symbol-Ansatz benˆtigt)
      MM_k := Matrix(NQ,NQ):
...
# Regressor-Matrix als Vektor hinschreiben. Dabei Null-Eintr√§ge herausfinden und nicht weiter ber√ºcksichtigen.
# Dadurch wird die obere rechte Dreiecksform der Regressor-Matrix ausgenutzt.
# Platzhalter-Vektor f√ºr alle Eintr√§ge der Regressor-Matrix
Reg_Vector := Matrix(ColumnDimension(tau_regressor_s)*RowDimension(tau_regressor_s), 1):
# Gelenkmoment in Abh√§ngigkeit des Regressor-Vektors (RV)
tauJ_RV := Matrix(NQJ, 1):
kk := 0: # Z√§hler f√ºr laufende Nummer des Regressor-Vektors
for i from 1 to ColumnDimension(tau_regressor_s) do
  for j from 1 to RowDimension(tau_regressor_s)-6 do
    ...
    if tau_regressor_s(j+6,i) <> 0 then # Eintrag ungleich Null
    	 ...
    	 # Eintrag f√ºr Regressor-Vektor so schreiben, dass er in einer Matlab-Funktion aus der Regressor-Matrix gebildet werden kann
...
# K√ºrzen des Regressor-Vektors auf die tats√§chlich benutzten Eintr√§ge
