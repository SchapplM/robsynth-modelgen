...
read sprintf("../codeexport/%s_tree_floatb_definitions", robot_name, base_method_name):
...
save Paramvec2, sprintf("../codeexport/%s_minimal_parameter_vector_maple", robot_name):
if codegen_act then
   MatlabExport(Paramvec2, sprintf("../codeexport/%s_minimal_parameter_vector_matlab.m", robot_name), codegen_opt):
end if;
...
save u_ges_minpar, sprintf("../codeexport/%s_energy_potential_fixb_regressor_minpar_maple.m", robot_name):
# Export
if codegen_act then
  MatlabExport(convert_t_s(t_ges_minpar), sprintf("../codeexport/%s_energy_kinetic_fixb_regressor_minpar_matlab.m", robot_name), codegen_opt):
end if:
if codegen_act then
  MatlabExport(convert_t_s(u_ges_minpar), sprintf("../codeexport/%s_energy_potential_fixb_regressor_minpar_matlab.m", robot_name), codegen_opt):
end if:
