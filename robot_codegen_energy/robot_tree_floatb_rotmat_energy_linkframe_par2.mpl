...
Trf_c := Trf_c:
read sprintf("../codeexport/%s_velocity_linkframe_floatb_%s_maple.m", robot_name, base_method_name):
omega_i_i := omega_i_i: 
rD_i_i := rD_i_i:
# Calculate Energy
...
if codegen_act then
  MatlabExport(T_s, sprintf("../codeexport/%s_energy_kinetic_floatb_%s_linkframe_par2_matlab.m", robot_name, base_method_name), codegen_opt):
end if:
# Fixed Base
T_s_fixb:=T_s:
...
if codegen_act then
  MatlabExport(T_s_fixb, sprintf("../codeexport/%s_energy_kinetic_fixb_linkframe_par2_matlab.m", robot_name), codegen_opt):
end if:
