...
if [ "$gen_single_jacobi_files" -eq "1" ]; then
  ...
  # Reihenfolge:
  # (1) analytisch, transl. (nur für einen Körper)
  # (2) analytisch, rot.
  # (3) geometrisch, rot.
  ...
  do
    ...
    if [ "$jacart" -eq "1" ] || [ "$jacart" -eq "2" ] || [ "$jacart" -eq "6" ] || [ "$jacart" -eq "7" ] || [ "$jacart" -eq "8" ] || [ "$jacart" -eq "11" ]; then # Vollständige Matrix: Position als Eingang
      input_r=true
    else
      input_r=false
    fi;
...
# Oder eine Datei für die Jacobi-Matrizen aller Segmente erstellen
  ...
  # (4) analytisch, translatorischer Teil
  # (5) geometrisch, Zeitableitung, rot.
  # (6) analytisch, Zeitableitung, rot.
  # (7) analytisch, Zeitableitung, translatorisch.
  # (8) geometrisch (vollständig)
  # (9) analytisch (vollständig)
  # (10) geometrisch, Zeitableitung (vollständig)
  # (11) analytisch, Zeitableitung (vollständig)
  ...
  do
    ...
    if [ "$jacart" -eq "5" ] || [ "$jacart" -eq "6" ] || [ "$jacart" -eq "7" ] || [ "$jacart" -eq "10" ] || [ "$jacart" -eq "11" ]; then # Zeitableitung: Geschwindigkeit als Eingang
      input_qD=true
    else
      input_qD=false
    fi;
    
    if [ "$jacart" -eq "4" ] || [ "$jacart" -gt "6" ]; then # translatorisch oder Gesamtmatrix
      input_r=true
    else
      input_r=false
    fi;
    ...
    if [ "$jacart" -lt "8" ]; then # Kopieren des symbolischen Codes der einzelnen Teilkörper für rot und transl
      for (( ib=0; ib<$robot_NL; ib++ ))
      do
        if [ "$ib" -eq "0" ]; then
          printf "if link_index == ${ib}\n" >> $zieldat
