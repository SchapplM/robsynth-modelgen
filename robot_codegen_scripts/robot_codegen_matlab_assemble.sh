...
do
  ...
  do
    ...
    zieldat=$repo_pfad/codeexport/${robot_name}_coriolismat_floatb_${basemeth}_par${dynpar}_matlab.m
    if ! [ -f $zieldat ]; then
      echo "${zieldat##*/} existiert nicht. Versuche Zusammenzusetzen."
      # Prüfe, ob alle Einzel-Dateien vorhanden sind
      vollst=1
      ...
      for (( j=1; j<=( $(($robot_NJ+6)) ); j++ )) do
        teildat=$repo_pfad/codeexport/${robot_name}_coriolismat_floatb_${basemeth}_${i}_${j}_par${dynpar}_matlab.m
        if ! [ -f $teildat ]; then
          vollst=0
          echo "Code in ${teildat##*/} nicht vorhanden."
          break 2
