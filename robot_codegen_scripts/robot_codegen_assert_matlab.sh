...
# Gelenkwinkel
echo "assert(isa(qJ,'double') && isreal(qJ) && all(size(qJ) == [$robot_NQJ 1]), ...
  '%FN%: qJ has to be [${robot_NQJ}x1] double');" > $tmp_pfad/robot_matlabtmp_assert_qJ.m
echo "assert(isa(qJD,'double') && isreal(qJD) && all(size(qJD) == [$robot_NQJ 1]), ...
  '%FN%: qJD has to be [${robot_NQJ}x1] double');" > $tmp_pfad/robot_matlabtmp_assert_qJD.m
echo "assert(isa(qJDD,'double') && isreal(qJDD) && all(size(qJDD) == [$robot_NQJ 1]), ...
