...
# Basis
echo "assert(isa(V_base,'double') && isreal(V_base) && all(size(V_base) == [6 1]))" > $tmp_pfad/robot_matlabtmp_assert_vB.m
echo "assert(isa(VD_base,'double') && isreal(VD_base) && all(size(VD_base) == [6 1]))" > $tmp_pfad/robot_matlabtmp_assert_vBD.m
...
echo "assert(isa(Iges_num_mdh,'double') && isreal(Iges_num_mdh) && all(size(Iges_num_mdh) == [$robot_NL 6]), ...
  'Iges_num_mdh has to be [${robot_NL}x6] double'); " > $tmp_pfad/robot_matlabtmp_assert_Ic.m
