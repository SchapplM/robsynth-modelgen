...
printf("Winkel delta8 berechnet. %s. CPU-Zeit bis hier: %1.2fs.", FormatTime("%Y-%m-%d %H:%M:%S"), time()-st):
# Berechne delta6
# Gl. (m3.8)
# Siehe auch: KAS5_m3_sym_codegen_kinematic_constraints_hilfsberechnungen.mw
...
for i from 1 to RowDimension(kintmp_subsexp) do
  MatlabExport(kintmp_subsexp(i,2), sprintf("../codeexport/KAS5_kinematik_parallel_kintmpsubsexp_%d_matlab_opt.m", i), 1):
end do:
...
read sprintf("../codeexport/%s_kinematic_constraints_maple.m", robot_name):
...
for i from 1 to RowDimension(kintmp_s) do
  MatlabExport(kintmp_qs(i), sprintf("../codeexport/KAS5_kinematik_parallel_kintmp_%d_matlab_opt.m", i), codegen_opt):
end do:
